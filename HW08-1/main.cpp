...
public: /// Ctor/Dtor
    CCamera(float fov = 90, float aspect = 1, float _near = 0.3, float _far = 1000) : CEntity(), fov(fov), aspect(aspect), zNear(_near), zFar(_far) {}
...
void Init() {
    ...
    box.Translate(50, 20, 16);

    cam.Translate(150, 45, 150);
    cam.Target = &box;
...
void KeyboardFunc(unsigned char key, int x, int y) {
    std::cout << "== Keyboard" << "\n";
    Vec3::Type origin;
    box.Transform.GetPosition(origin);
    switch(key) {
        case 'a':
            cam.RotateY(-10, origin);
            break;
        case 'd':
            cam.RotateY(10, origin);
            break;
